...
* Modified for path counting by Phannarat Horng
**/
...
class FloydWarshall {
    ...
    */
    private function __floydwarshall () {
        ...
        for ( $k = 0; $k < $this->nodes; $k++ ) {
            for ( $i = 0; $i < $this->nodes; $i++ ) {
                for ( $j = 0; $j < $this->nodes; $j++ ) {
					if ($this->dist[$i][$j] == ($this->dist[$i][$k] + $this->dist[$k][$j]) && k!=j && k!=i){
						$this->pathNum[$i][$j] += 1;
					} else if ($this->dist[$i][$j] > ($this->dist[$i][$k] + $this->dist[$k][$j])) {
    ...
    */
    ...
    /**
    * Get number of shortest paths between point a to b.
    *
    * @param integer $i
    * @param ingeger $j
    * @return array Returns an array of numbers.
    */
    public function get_pathNum($i, $j) {
        return $this->pathNum[$i][$j]/2;
    }
